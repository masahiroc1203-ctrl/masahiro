...
`cd home_energy && python watch_channels.py 300` を動かしながら、エアコン4台・IH等を1つずつON/OFFし、値が動いたチャンネルを記録する。2026-07-19 15:03時点の観測: ch1=45W, ch2=23W, ch3=0, ch4=270W, ch5=0, ch6=0（マスタ約900W消費・太陽光約800W発電）。エアコン全停止ならch1/ch2/ch4は待機電力または常時負荷。

### 未解決事項

- 6チャンネル←→回路のマッピング — watch_channels.py によるON/OFFテストで特定（上記手順）
- エネファーム（燃料電池）の有無と、太陽光との併設状況 — ユーザーに要確認。ECHONET上は太陽光(0x0279)のみ応答、燃料電池クラス(0x027C)は未検出
