...
- [ ] HEMS端末の型番を確認する（AiSEG2想定・未確定）
- [ ] HEMSエクスポートCSVの実物を確認し、`electricity_hourly.csv` 形式への変換スクリプト `home_energy/convert_hems.py` を追加
...
- [ ] （自動化・方式A）常時稼働機（RasPi等）でHEMSをLANポーリングし日次で git push する収集スクリプト
...
- HEMS端末の型番（Panasonic AiSEG2か？）— 方式Aの取得スクリプト設計に必要。端末画面かスマホアプリ名で確認
- HEMSエクスポートCSVの実際の形式（横持ち/縦持ち、30分値/1時間値）— 変換スクリプトの仕様が決まる
